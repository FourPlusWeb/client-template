...
При всеки push GitHub Actions проверява `pnpm lint`, `pnpm typecheck`, `pnpm build`. Visual regression за `@fourplusweb/ui` компонентите се изпълнява на library-level (UI пакет release), не в консуматорския template. Клиентски сайтове могат да добавят собствени Playwright e2e конфигурации по нужда.
